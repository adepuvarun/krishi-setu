...
crop_recommendation_model_path = 'D:\\VARU WORKSPACE\\varun files\\4-1\\major\\ArogyaKrishi-main\\model\\RandomForest.pkl'
crop_recommendation_model = pickle.load(
    open(crop_recommendation_model_path, 'rb'))


soil_type_prediction_model_path = 'D:\\VARU WORKSPACE\\varun files\\4-1\\major\\ArogyaKrishi-main\\model\\DenseNet121v2_95.h5'
...
model_path = "D:\\VARU WORKSPACE\\varun files\\4-1\\major\\ArogyaKrishi-main\\model\\SoilNet_93_86.h5"
...
def generate_fertilizer_recommendation(disease_name, latitude, longitude, api_key):
    ...
    # Get base recommendation from dictionary
    if disease_name in recommendations:
        base_recommendation = recommendations[disease_name]
    else:
        base_recommendation = {
            'fertilizer': 'General-purpose Fertilizer',
            'details': 'Apply 50g per plant every two weeks.',
            'application_method': 'Spread around base and water.'
        }
    ...
    if weather_data:
        ...
        # Adjust recommendation dynamically
        if humidity > 80:
            base_recommendation['details'] += f" High humidity detected ({humidity}%), increase monitoring."
        if temperature > 30:
            base_recommendation['details'] += f" High temperature ({temperature}°C) may accelerate disease spread."
...
def fert_recommend():
    ...
    df = pd.read_csv('D:\\VARU WORKSPACE\\varun files\\4-1\\major\\ArogyaKrishi-main\\data\\fertilizer.csv')
